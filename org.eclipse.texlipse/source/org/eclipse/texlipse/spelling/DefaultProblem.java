...
public class DefaultProblem extends CategorizedProblem {
	...
	// cannot directly point to IJavaModelMarker constants from within batch
	// compiler
	...
	public String errorReportSource(char[] unitSource) {
		...
		while ((c = unitSource[begin]) == ' ' || c == '\t')
			begin++;
		// while ((c = unitSource[end]) == ' ' || c == '\t') end--; TODO (philippe)
		// should also trim right, but all tests are to be updated
