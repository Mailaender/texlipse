...
import org.eclipse.texlipse.spelling.engine.DefaultSpellChecker;
import org.eclipse.texlipse.spelling.engine.ISpellCheckIterator;
...
public class SpellCheckIterator implements ISpellCheckIterator {
	...
	protected String nextToken() {
		...
		if (fNext - fPrevious > 0) {
			...
			if (fSuccessor != BreakIterator.DONE && fContent.charAt(fPrevious) == ITexTagConstants.TEX_TAG_PREFIX) {
			...
			} else if (fSuccessor != BreakIterator.DONE && fContent.charAt(fPrevious) == ITexTagConstants.TEX_TAG_PREFIX && (Character.isLetter(fContent.charAt(fNext)) || fContent.charAt(fNext) == '/')) {

				if (fContent.startsWith(ITexTagConstants.TEX_CLOSE_PREFIX, fPrevious))
					nextBreak();

				nextBreak();

				if (fSuccessor != BreakIterator.DONE && fContent.charAt(fNext) == ITexTagConstants.TEX_TAG_POSTFIX) {
			...
			} else if (fSuccessor != BreakIterator.DONE && fContent.charAt(fPrevious) == ITexTagConstants.TEX_ENTITY_START && (Character.isLetter(fContent.charAt(fNext)))) {
				nextBreak();
				if (fSuccessor != BreakIterator.DONE && fContent.charAt(fNext) == ITexTagConstants.TEX_ENTITY_END) {
					nextBreak();
					if (isToken(fContent.substring(fPrevious, fNext), ITexTagConstants.TEX_ENTITY_CODES)) {
						skipTokens(fPrevious, ITexTagConstants.TEX_ENTITY_END);
						update= true;
					} else
						token= fContent.substring(fPrevious, fNext);
				} else
