 ...
 * @since 3.0
 */
public class WordCorrectionProposal implements ITexCompletionProposal {
	...
	public static String getHtmlRepresentation(final String string) {
		...
		for (int offset= length - 1; offset >= 0; offset--) {
			...
			for (int index = 0; index < ITexTagConstants.TEX_ENTITY_CHARACTERS.length; index++) {

				if (string.charAt(offset) == ITexTagConstants.TEX_ENTITY_CHARACTERS[index]) {

					buffer.replace(offset, offset + 1, String.valueOf(ITexTagConstants.TEX_ENTITY_CODES[index]));
					break;
				}
			}
		}
	...
	public String getDisplayString() {
		return MessageFormat.format(TexUIMessages.Spelling_correct_label, new String[] { fWord });
	}
	...
	public Image getImage() {
		return TexlipsePlugin.getImage("rename");
	}
