 ...
 *******************************************************************************/
package org.eclipse.texlipse.spelling.engine;
...
import org.eclipse.texlipse.spelling.BasicElementLabels;
...
public abstract class AbstractSpellDictionary implements ISpellDictionary {
	...
	protected synchronized boolean load(final URL url) {
		...
		if (url != null) {
			...
			try {
						...
						while (doRead) {
							try {
								...
								String message = MessageFormat.format(
										TexUIMessages.AbstractSpellingDictionary_encodingError,
										new String[] { word, decoder.replacement(), BasicElementLabels.getURLPart(url.toString()) });
								IStatus status = new Status(IStatus.ERROR, TexlipsePlugin.getPluginId(), IStatus.OK,
										message, ex);
								TexlipsePlugin.log(status);
				...
				if (line > 0) {
					String message = MessageFormat.format(TexUIMessages.AbstractSpellingDictionary_encodingError,
							new Object[] { Integer.valueOf(line), BasicElementLabels.getURLPart(url.toString()) });
					IStatus status = new Status(IStatus.ERROR, TexlipsePlugin.getPluginId(), IStatus.OK, message,
							exception);
					TexlipsePlugin.log(status);
				} else
	...
	protected String getEncoding() {
		String encoding = TexlipsePlugin.getDefault().getPreferenceStore()
